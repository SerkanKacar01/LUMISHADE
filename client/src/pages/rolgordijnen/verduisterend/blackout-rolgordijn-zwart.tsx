...
export default function BlackoutRolgordijnZwartPage() {
  ...
  // Get current price based on selected width for verduisterend
  const currentPrice = getPriceByWidth('verduisterend', selectedWidth);
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (!isNaN(searchValue)) {
      ...
      const closestWidth = widthOptions.reduce((prev, curr) =>
        Math.abs(curr - searchValue) < Math.abs(prev - searchValue)
          ? curr
          : prev,
      );
                      ...
                      >
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-xs opacity-75">
                            €{getPriceByWidth('verduisterend', width).toFixed(2)}
                          </span>
                        </div>
                      </button>
                    ))}

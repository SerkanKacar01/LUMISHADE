...
export default function BlackoutRolgordijnZwartPage() {
  ...
  // Get current price based on selected width (using a higher base price for blackout)
  const basePrice = getPriceForWidth(selectedWidth) + 25; // Add €25 for blackout functionality
  const currentPrice = basePrice;
  const totalPrice = currentPrice * quantity;

  const handleAddToCart = () => {
    console.log("Adding to cart:", {
      product: "Blackout Rolgordijn Zwart",
      quantity,
      operatingSide,
  ...
  // Get all available widths for scrollable selection
  const widthOptions = getAvailableWidths();

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const width = parseInt(searchWidth);
    if (width && widthOptions.includes(width)) {
      setSelectedWidth(width);
      const element = document.querySelector(`[data-width="${width}"]`);
      if (element) {
            ...
            <div className="flex items-center gap-4 mb-4">
              <Link to="/rolgordijnen/verduisterend">
                <Button variant="ghost" size="sm" className="flex items-center gap-2">
                  ...
                  Terug naar verduisterende rolgordijnen
                </Button>
              </Link>
            </div>
            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">
              Blackout Rolgordijn Zwart
            </h1>
            <p className="text-lg text-gray-600">
              Volledig verduisterende zwarte rolgordijn voor optimale privacy en lichtblokkering.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <img
                ...
                alt="Blackout Rolgordijn Zwart"
                className="w-full h-96 md:h-[500px] object-cover rounded-lg shadow-lg"
              />
              
              {/* Product Features */}
              <div className="bg-white p-6 rounded-lg shadow-sm">
                <h3 className="text-xl font-semibold text-gray-900 mb-4">Product eigenschappen</h3>
                <ul className="space-y-2 text-gray-700">
                  <li className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-primary rounded-full"></span>
                    100% verduisterende blackout stof
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-primary rounded-full"></span>
                  ...
                  <li className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-primary rounded-full"></span>
                    Maximale privacy bescherming
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-primary rounded-full"></span>
                    Perfect voor slaapkamers
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="text-center lg:text-left">
                ...
                <p className="text-gray-600">Prijs per stuk (excl. BTW)</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <form onSubmit={handleSearchSubmit} className="flex gap-2 mb-4">
                  <div className="relative flex-1 max-w-xs">
                    ...
                    <Input
                      ...
                      min="40"
                      max="280"
                      placeholder="Ga naar breedte (bijv. 150)"
                      value={searchWidth}
                ...
                <div className="relative">
                  <div
                    className="width-selector-scroll flex gap-3 overflow-x-auto pb-4 scroll-smooth"
                    id="width-selector-container"
                  >
                        ...
                        >
                          <div className="flex flex-col items-center">
                            ...
                            <span className="text-xs opacity-75">
                              €{(getPriceForWidth(width) + 25).toFixed(2)}
                            </span>
                          </div>
                        </button>
                      ))}
                ...
                <div className="mt-3 mb-4 text-center md:hidden">
                  <p className="text-sm text-gray-500">
                    Veeg naar links of rechts om je gewenste breedte te
                    selecteren.
                  </p>
                </div>

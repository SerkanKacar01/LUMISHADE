...
export default function ProfessionalBlackoutCharcoalPage() {
  ...
  const handleAddToCart = () => {
    console.log("Adding to cart:", {
      product: "Professional Blackout Charcoal",
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white rounded-lg shadow-sm p-6 space-y-8">
            <div className="space-y-6">
              <h1 className="text-3xl lg:text-4xl font-bold text-gray-900">Professional Blackout Charcoal</h1>
              <div className="aspect-w-16 aspect-h-12 lg:aspect-w-4 lg:aspect-h-3">
                <img src="https://images.unsplash.com/photo-1615873968403-89e068629265?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=800&q=80" alt="Professional Blackout Charcoal" className="w-full h-64 lg:h-80 object-cover rounded-lg" />
              </div>
                    ...
                    {widthOptions.map((width) => (
                      <button key={width} data-width={width} onClick={() => setSelectedWidth(width)} className={`width-button-consistent flex-shrink-0 min-w-[80px] md:min-w-[90px] px-3 md:px-4 py-2 text-sm md:text-base font-medium rounded-lg border transition-all duration-200 touch-target ${selectedWidth === width ? "selected bg-primary text-white border-primary shadow-lg" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50 hover:border-gray-400 hover:shadow-md active:scale-95"}`}>
                        <div className="flex flex-col items-center">
                          <span>{width}cm</span>
                          <span className="text-xs opacity-75">€{getPriceByWidth('verduisterend', width).toFixed(2)}</span>
                        </div>
                      </button>
                    ))}
                  </div>
